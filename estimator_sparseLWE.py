...
def drop_and_solve_hyb(f, n, alpha, q, offset, secret_distribution, 
                        success_probability=0.99, **kwds):
    ...
    k = step_size
    C = binomial(n, h)
    B = (1 + 1/sqrt(2 * pi)) * alpha * q * q / (sqrt(2) * (2 ** offset))

    while k < n:
        current = f(n - k, alpha, q, k, offset, success_probability = success_probability,
                    secret_distribution=secret_distribution, **kwds)
        if current["red"] == oo:
            break        
            ...
            while h1 < min(k, h):
                h1 += 1
                num_table += binomial(k1, h1) * (b-a) ** h1
                repeat = round(2 * log(num_table, 2) / (1 - 4 * B / q))
                if num_table * repeat > cost_lat:
                    num_table -= binomial(k1, h1) * (b-a) ** h1
                    h1 -= 1
                    repeat = round(2 * log(num_table, 2) / (1 - 4* B / q))
                    break

            cost_mem = num_table * repeat

            cost_query = 0
            h2 = 0
            while cost_query + binomial(k2, h2 + 1) * (b-a)**(h2 + 1) * (max(1, 2 ** (repeat * 4 * B / q))) < cost_lat:
                h2 += 1
                cost_query += binomial(k2, h2) * (b-a)** h2 * (max(1, 2 ** (repeat * 4 * B / q)))
                if h2 >= min(k, h - h1):
                    break
            ...
            if probability == 0:
                ...
                best_k = current
                best_k["rop"] = total_cost  
                best_k["repeat"] = repeat     
                # best_k["one_query"] = int(max(1, 2 ** (repeat * 4 * B / q)))    
                best_k["mem"] = num_table
                best_k["k"] = k
                best_k["h1"] = h1
                best_k["h2"] = h2
                best_k = best_k.reorder(["rop"])  
                h2 += 1
...
def dual_scale_hyb(n, alpha, q, k, offset, secret_distribution, success_probability=0.99,
...
def hybrid_dual(n, alpha, q, secret_distribution, m=oo, reduction_cost_model = reduction_default_cost):
    """
    Hybrid dual attack as described in [IEEEAccess:CHHS19]_
    Note that the memory requirements are bounded at 2**80.
    ...
    EXAMPLE::
        ...
        sage: from estimator import hybrid_dual
        sage: n = 2048
        sage: q = 2**54
        sage: alpha = 8.0/q
        sage: secret = ((-1,1),64)
        sage: hybrid_dual(n, alpha, q, secret_distribution=secret)
                rop:  2^104.3
                m:   2^11.2
                red:   2^92.3
                delta_0: 1.006079
    ...
    .. [IEEEAccess:CHHS19] Cheon, J. H., Hhan, M., Hong, S., & Son, Y.  (2019).  A Hybrid 
        of Dual and Meet-in-the-Middle Attack on Sparse and Ternary Secret LWE. In IEEE 
        Access, vol. 7, pp. 50595--50604. 2019.
    """
    
    #offset = ceil(log(2*sqrt(2)*(1+1/sqrt(pi))*alpha*q, 2))
    offset = 7.0
    ...
    while depth <= 3 and offset <= log(q,2):
        res = mitm_hyb(n, alpha, q, offset, secret_distribution=secret_distribution, reduction_cost_model=reduction_cost_model)
        if best is None or (res is not None and res["rop"] < best["rop"]):            
            best = res
            if depth >= 1:
                step = step/2
            offset += step
        elif res is None or res["rop"] >= best["rop"]:
            step = step/2
            depth += 1
            offset -= step
    return best
...
def hybrid_primal(n, alpha, q, secret_distribution, m = oo, success_probability=0.99, 
               reduction_cost_model=reduction_default_cost, 
               init_delta_P = None, opt_GSA = False, verbose=False):
    
    print("NOTE: Hybrid-Primal estimation may takes LONG LONG time (even few hours) for large HE parameters.")
    print("\n")
    ...
    if init_delta_P == None:
        ...
        best = duald(n=n, alpha=alpha, q=q, m=m, secret_distribution=secret_distribution, reduction_cost_model=reduction_cost_model)
